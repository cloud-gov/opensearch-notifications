...
internal object UserAccessManager : UserAccess {
    ...
    override fun getSearchAccessInfo(user: User?): List<String> {
        if (user == null || !PluginSettings.isRbacEnabled() || user.roles.contains(ADMIN_ROLE)
    ...
    override fun doesUserHaveAccess(user: User?, access: List<String>): Boolean {
        ...
        // User has access to resource if resource is public i.e. no access roles attached, user is
        // an admin user or there is any intersection
        // between user backend roles and access roles
        return access.isEmpty() ||
            user.roles.contains(ADMIN_ROLE) ||
            user.backendRoles.equals(access)
    }
}
